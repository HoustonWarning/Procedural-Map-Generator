...
public static class MeshGenerator {
    ...
    public static MeshData TerrainMeshGenerator(float[,] heightMap, MeshSettings meshSettings, int levelOfDetail)
    {
        ...
        for (int y = 0; y < numVertsPerLine; y++)
        {
            for(int x = 0; x < numVertsPerLine; x++)
            {
                ...
                {
                    bool isOutOfMeshVertex = y == 0 || y == numVertsPerLine - 1 || x == 0 || x == numVertsPerLine - 1;
                    bool isMeshEdgeVertex = (y == 1 || y == numVertsPerLine - 2 || y == 1 || y == numVertsPerLine - 2)&&!isOutOfMeshVertex;
                    bool isMainVertex = (x - 2) % skipIncrement == 0 && (y - 2) % skipIncrement == 0 && !isOutOfMeshVertex && !isMeshEdgeVertex;
                    bool isEdgeConnectVertex = (y == 2 || y == numVertsPerLine - 3 || x == 2 || x == numVertsPerLine - 3) && !isOutOfMeshVertex && !isMeshEdgeVertex && !isMainVertex;
                    ...
                    if (isEdgeConnectVertex)//stitching the edge vertices to the main vertex
                    {
                    ...
                    bool createTriangle = x < numVertsPerLine - 1 && y < numVertsPerLine - 1 && (!isEdgeConnectVertex || (x != 2 && y != 2));

                    if (createTriangle)
                    {
                        int currenIncrement = (isMainVertex && x != numVertsPerLine - 3 && y != numVertsPerLine - 3) ? skipIncrement : 1;

                        int a = vertexIndecesMap[x, y];
                        int b = vertexIndecesMap[x + currenIncrement, y];
                        int c = vertexIndecesMap[x, y + currenIncrement];
                        int d = vertexIndecesMap[x + currenIncrement, y + currenIncrement];
...
public class MeshData
{
    ...
    public MeshData(int numVertsPerLine, int skipIncrement, bool useFlatShading)
    {
        ...
        int numEdgeConnectVertices = (skipIncrement - 1) * (numVertsPerLine - 5) / skipIncrement * 4;
        int numMainVerticesPerLine = (numVertsPerLine - 5) / skipIncrement + 1;
        int numMainVertices = numMainVerticesPerLine * numMainVerticesPerLine;


        vertices = new Vector3[numVertsPerLine * numVertsPerLine];
        uvs = new Vector2[numVertsPerLine * numVertsPerLine];
    ...
    void FlatShading()
    {
        Vector3[] flatsShadedVertices = new Vector3[triangles.Length];
        Vector2[] flatShadedUvs = new Vector2[triangles.Length];
        for(int i = 0; i < triangles.Length; i++)
        {
            flatsShadedVertices[i] = vertices[triangles[i]];
            flatShadedUvs[i] = uvs[triangles[i]];
            triangles[i] = i;
        }
        vertices = flatsShadedVertices;
        uvs = flatShadedUvs;
    }
